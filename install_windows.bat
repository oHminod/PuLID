...
"%VENV_PYTHON%" -c "from pulid_app.models.text_embedding import _cuda_dll_search_path; dll_context = _cuda_dll_search_path('cuda'); dll_context.__enter__(); import llama_cpp; info = llama_cpp.llama_print_system_info().decode(); assert 'CUDA' in info, 'Backend CUDA absent de llama-cpp-python'; print('llama-cpp-python CUDA OK :', llama_cpp.__version__); dll_context.__exit__(None, None, None)"
